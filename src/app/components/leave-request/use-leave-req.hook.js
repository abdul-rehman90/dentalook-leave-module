...
export default function useLeaveReq() {
  ...
  const clinicByRegionalManager = async () => {
    try {
      const response = await axiosInstance.get(
        `api/v1/clinic-by-regional-manager/${provinceId}`
      );
      if (response.status === 200) {
        setRegionalManagers(response?.data?.regional_managers);
        console.log(
          response?.data?.regional_managers,
          '...response?.data?.regional_managers'
        );
      }
    } catch (error) {
  ...
  const getProviders = async () => {
    try {
      const response = await axiosInstance.get(
        `api/v1/provider-by-clinic/${clinicId}`
      );
      if (response.status === 200) {
  ...
  const hanleGetAllRM = async () => {
    try {
      const response = await axiosInstance.get(
        `api/v1/admin/all-users/?role=RM`
      );
      if (response.status === 200) {
  ...
  const handleGetAllProviders = async () => {
    try {
      const response = await axiosInstance.get(
        `api/v1/admin/all-users/?role=Provider`
      );
      if (response.status === 200) {
  ...
  const filterClinicByRM = (id) => {
    const data = allClinics?.filter(
      (clinic) =>
        clinic.regional_manager && clinic.regional_manager[0]?.id === id
    );
    setClinics(data);
  };
  ...
  const handleChangeClinic = (id, options) => {
    if (id) {
      ...
      setRegionalManagersId(
        currentRM?.regional_manager && currentRM?.regional_manager[0]?.id
      );
  ...
  const handleChangeProviderName = (id, options) => {
    ...
    const currentClinic = allClinics?.find(
      (clinic) => clinic.id === options?.clinics?.id
    );
